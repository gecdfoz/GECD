...
Aqui vamos trabalhar na janela que o RStudio abriu com o nosso novo projeto criado.

![](images/guia_git.png)

Primeiro vamos executar o comando `use_git()`do pacote usethis. Ao realizar a ação, o programa irá perguntar se você deseja '*commitar*' (salvar) os arquivos que estão dentro da pasta correspondente ao seu repositório. Na sequência também irá perguntar se é para realizar o commit com a frase "*initial commit*". Para ambos os casos, é necessário digitar um número correspondendo a estar de acordo.
